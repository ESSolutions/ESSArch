            ...
            if os.path.isdir(path):
                for root, _dirs, files in os.walk(path):
                    for f in files:
                        ...
                        _convert(fpath, rootdir, tool, options, self.get_processtask())
            else:
                _convert(path, rootdir, tool, options, self.get_processtask())
    else:
        filepath = os.path.join(rootdir, pattern)
        tool.run(filepath, rootdir, options, self.get_processtask())
        if tool.delete_original:
            os.remove(filepath)
