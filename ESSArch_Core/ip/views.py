...
class InformationPackageViewSet(viewsets.ModelViewSet):
    ...
    def actiontool(self, request, pk=None):
        ...
        workflow_spec_to_run, workflow_spec_to_save = [{
            "step": True,
            "name": "Action tool",
            "children": []
        }]

        #serializer.validated_data['action_workflow_name']
        #serializer.validated_data['action_workflow_status']
        ...
        for converter in serializer.validated_data['actions']:
            ...
            workflow_spec_to_run[0]['children'].append({
                "name":
                "ESSArch_Core.fixity.action.tasks.Action",
                "label":
                tool_name,
                ...
                ip.object_path,
                "args":
                [tool_name, pattern, ip, options,
                 request.data.get('purpose')]
            })

            workflow_spec_to_save[0]['children'].append({
                "name":
                "ESSArch_Core.fixity.action.tasks.Action",
                "label":
                tool_name,
            ...
            print("workflowspec2run", workflow_spec_to_run)
            print("workflowspec2save", workflow_spec_to_save)
            # Add action_workflow_profile = request.data.get('action_workflow_name')
            if request.data.get('action_workflow_name'):
                dct = {
                    'name': request.data.get('action_workflow_name'),
                    'profile_type': 'action_workflow',
                    'type': 'Action Workflow',
                    'status': request.data.get('action_workflow_status'),
                    'label': request.data.get('action_workflow_name'),
                    'specification': workflow_spec_to_save,
                },
                profile, _ = Profile.objects.update_or_create(name=dct['name'],
                                                              defaults=dct)
            else:
                workflow = create_workflow(workflow_spec_to_run,
                                           eager=False,
                                           ip=ip)
                workflow.run()""
            options = converter['options']
...
class InformationPackageReceptionViewSet(viewsets.ViewSet, PaginatedViewMixin):
    ...
    def _prepare(self, request, pk):
        ...
        reception = Path.objects.values_list(
            'value', flat=True).get(entity='ingest_reception')
        uip = Path.objects.get(
            entity="ingest_unidentified"
        ).value  # This does not do anything particular right now
