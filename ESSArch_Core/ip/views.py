...
class InformationPackageViewSet(viewsets.ModelViewSet):
    ...
    def annotate_generations(qs):
        lower_higher = InformationPackage.objects.filter(
            Q(aic=OuterRef('aic')), Q(Q(workareas=None) | Q(workareas__read_only=True))).order_by().values('aic')
        lower_higher = lower_higher.annotate(min_gen=Min('generation'), max_gen=Max('generation'))

        return qs.annotate(
            first_generation=InformationPackageViewSet.first_generation_case(lower_higher),
    ...
    def annotate_filtered_first_generation(self, qs, user):
        lower_higher = InformationPackage.objects.visible_to_user(user).filter(
            Q(Q(workareas=None) | Q(workareas__read_only=True)), aic=OuterRef('aic'),
        ).order_by().values('aic')
    ...
    def get_queryset(self):
        ...
        if not self.detail and view_type == 'aic':
            ...
            simple_outer = InformationPackage.objects.annotate(has_ip=Exists(
                simple_inner.only('id').filter(aic=OuterRef('pk')))).filter(package_type=InformationPackage.AIC,
                    has_ip=True)
            ...
            inner = inner.prefetch_related(Prefetch('profileip_set', queryset=profile_ips,))
            simple_outer = simple_outer.prefetch_related(Prefetch('profileip_set', queryset=profile_ips,))
            ...
            filtered = InformationPackage.objects.visible_to_user(user).filter(
                Q(Q(workareas=None) | Q(workareas__read_only=True)),).exclude(package_type=InformationPackage.AIC)
            ...
            outer = simple.filter(filtered_first_generation=True).prefetch_related(
                Prefetch('aic__information_packages', queryset=inner)).distinct()
        ...
        if self.detail:
            lower_higher = InformationPackage.objects.filter(
                Q(aic=OuterRef('aic')),
                Q(Q(workareas=None) | Q(workareas__read_only=True))).order_by().values('aic')
            lower_higher = lower_higher.annotate(min_gen=Min('generation'), max_gen=Max('generation'))
    ...
    def save_actiontool(self, request, pk=None):
        ...
        for converter in serializer.validated_data['actions']:
            ...
            workflow_spec[0]['children'].append({
                "name": "ESSArch_Core.fixity.action.tasks.Action",
                "label": tool_name,
                "args": [tool_name, pattern, options, request.data.get('purpose')]
            })
    ...
    def submit(self, request, pk=None):
        ...
        if ip.state != "Created":
            return Response("The IP (%s) is in the state '%s' but should be 'Created'" % (pk, ip.state), status=status.HTTP_400_BAD_REQUEST)
        ...
        if sd_profile is None:
            return Response("The IP (%s) has no submit description profile" % pk, status=status.HTTP_400_BAD_REQUEST)
    ...
    def first_generation_case(lower_higher):
        return Case(
            ...
            When(generation=Subquery(lower_higher.values('min_gen')[:1]), then=Value(True)), default=Value(False), output_field=BooleanField(),
        )
    ...
    def last_generation_case(lower_higher):
        return Case(
            When(aic__isnull=True, then=Value(True)),
            When(package_type=InformationPackage.AIC, then=Value(True)),
            When(generation=Subquery(lower_higher.values('max_gen')[:1]), then=Value(True)), default=Value(False), output_field=BooleanField(),
        )
    ...
    def destroy(self, request, pk=None):
        ...
        self.logger.info('Request issued to delete %s %s' % (ip.get_package_type_display(), pk), extra={'user': request.user.pk})
    ...
    def access(self, request, pk=None):
        ...
        ip_already_in_workarea = ip_workarea.exists() and (ip_workarea.filter(type=workarea_type).exists() or ingest_path == access_path)
    ...
    def create_dip(self, request, pk=None):
        ...
        dst = os.path.join(os.path.dirname(dip.object_path), dip.object_identifier_value + '.' + dip.get_container_format().lower(),)
        ...
        if ip.archived:
            ...
            if path != '':
                ...
                q = ElasticQ(
                    'bool',
                    should=[
                        ElasticQ('bool', must=[ElasticQ('term', **{'href': dirname}), ElasticQ('term', **{'name.keyword': basename})]),
                        ElasticQ('bool', must=[ElasticQ('term', **{'href': dirname}), ElasticQ('match', filename=basename)])
                    ]
                )
    ...
    def unlock_profile(self, request, pk=None):
        ...
        return Response({'detail': 'Unlocking profile with type "%s" in IP "%s"' % (ptype, ip.pk)})
    ...
    def validate(self, request, pk=None):
        ...
        step = ProcessStep.objects.create(name="Validation", information_package=ip)
...
class InformationPackageReceptionViewSet(viewsets.ViewSet, PaginatedViewMixin):
    ...
    def get_contained_packages(self, path):
        ...
        for xmlfile in self.find_xml_files(path):
            ...
            existing_ips = InformationPackage.objects.filter(object_identifier_value=ip_id).exclude(state="Submitted")
    ...
    def list(self, request):
        ...
        db_ip_ids = from_db.filter(object_identifier_value__in=[i['id'] for i in new_ips]).values_list(
            'object_identifier_value',
            flat=True
        )
    ...
    def _prepare(self, request, pk):
        ...
        existing_non_sip = InformationPackage.objects.filter(object_identifier_value=pk, ).exclude(package_type=InformationPackage.SIP).first()
        existing_sip = InformationPackage.objects.filter(object_identifier_value=pk, package_type=InformationPackage.SIP).first()
        ...
        if os.path.isdir(os.path.join(reception, pk)):
            ...
            if not os.path.isfile(container):
                logger.warning('Tried to receive IP with missing container file %s' % container, extra={'user': request.user.pk})
    ...
    def receive(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            workflow.save()
            workflow.run()
            logger.info('Started receiving {objid} from reception'.format(objid=ip.object_identifier_value), extra={'user': request.user.pk},)
    ...
    def transfer(self, request, pk=None):
        ...
        if existing_ip is not None:
            ...
            if not os.path.isfile(xmlfile):
                logger.warning('Tried to transfer IP with missing XML file %s' % xmlfile, extra={'user': request.user.pk})
                raise exceptions.NotFound('%s does not exist' % xmlfile)
            try:
                container = normalize_path(os.path.join(reception, self.get_container_for_xml(xmlfile)))
            except etree.LxmlError:
                logger.warning('Tried to transfer IP with invalid XML file %s' % xmlfile, extra={'user': request.user.pk})
                raise exceptions.ParseError('Invalid XML file, %s' % xmlfile)

            if not os.path.isfile(container):
                logger.warning('Tried to transfer IP with missing container file %s' % container, extra={'user': request.user.pk})
                raise exceptions.NotFound('%s does not exist' % container)
    ...
    def identify_ip(self, request):
        ...
        if not os.path.isfile(container_file):
            return Response({'status': '%s does not exist' % container_file}, status=status.HTTP_400_BAD_REQUEST)

        spec = json.loads(open(os.path.join(settings.BASE_DIR, 'templates/SDTemplate.json')).read())
        ...
        spec_data['_IP_CREATEDATE'] = timestamp_to_datetime(creation_date(container_file)).isoformat()
...
class WorkareaViewSet(InformationPackageViewSet):
    ...
    def get_queryset(self):
        ...
        workareas = Workarea.objects.all()
        workareas = WorkareaEntryFilter(
            data=workarea_params,
            queryset=workareas,
        ...
        if self.action == 'list' and view_type == 'aic':
            simple_inner = InformationPackage.objects.visible_to_user(user).annotate(
                workarea_exists=Exists(workareas.filter(
                    ip=OuterRef('pk')))).filter(workarea_exists=True, active=True)
            ...
            filtered = InformationPackage.objects.visible_to_user(user).annotate(
                workarea_exists=Exists(workareas.filter(ip=OuterRef('pk')))).filter(
                    workarea_exists=True, active=True).exclude(package_type=InformationPackage.AIC)
        ...
        if self.action == 'retrieve':
            lower_higher = InformationPackage.objects.filter(Q(aic=OuterRef('aic'))).order_by().values('aic')
            lower_higher = lower_higher.annotate(min_gen=Min('generation'), max_gen=Max('generation'))

            qs = InformationPackage.objects.visible_to_user(user).filter(active=True, )
